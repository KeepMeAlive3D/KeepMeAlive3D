...
export function pullLimitsUp(currentObject: Object3D, scene: Scene) {
  const limits = currentObject.children.filter(
    (x) => x.name.startsWith("limit_move_") || x.name.startsWith("limit_rot_")
  );
...
function getRotationByLimits(
  ...
  (object.parent ?? scene)
    .getWorldQuaternion(worldTransformationMatrix)
    .invert();

  return step.multiply(worldTransformationMatrix);
}

function getPositionByLimits(
  object: Object3D,
  scene: Scene,
  upperLimit: Object3D,
  lowerLimit: Object3D,
  ...
  const targetLocation = currentLowerWorldPosition.lerp(
    currentUpperWorldPosition,
    percentage
  );
...
function getUpperAndLowerLimit(currentObject: Object3D, scene: Scene) {
  const limits = (currentObject.parent ?? scene).children.filter((node) => {
    return (
      ...
      node.userData["target"] === currentObject.uuid
    );
  });
  ...
  const upper = limits.find(
    (x) => x.name.endsWith("_up") || x.name.endsWith("_max")
  );
  const lower = limits.find(
    (x) => x.name.endsWith("_down") || x.name.endsWith("_min")
  );

  if (!upper || !lower) {
    throw new Error(
      "Did not find upper and lower limit for object " + currentObject.name
    );
  }
