...
import { uploadParticipantEventLog } from "@/scene/dt/eventlogs/data.ts";

export function CreateEventLogDialog({setOpen, setRefresh, refresh, pId}: {
  ...
  function upload(values: z.infer<typeof formSchema>) {
    setLoading(true)
    uploadParticipantEventLog(Number(dtId), Number(refId), pId, values.file)
      .then(() => {
        setOpen(false)
        setRefresh(!refresh)
      })
    ...
    <DialogContent className="sm:max-w-[425px]">
      <Form {...form}>
        <form onSubmit={form.handleSubmit(upload)}>
          <DialogHeader>
            <DialogTitle>Upload a Event log {pId}</DialogTitle>
            <DialogDescription>
