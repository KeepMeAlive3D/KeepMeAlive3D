...
function Animator() {
  ...
  const animationCallback = useCallback((msg: PositionEventMessage) => {
    ...
    if (selectedObject) {
      console.debug("Moving object from " + selectedObject.position.toArray().join(";") + " to " + msg.message.position.x + ";" + msg.message.position.y + ";" + msg.message.position.z);
      selectedObject.position.set(selectedObject.position.x, selectedObject.position.y, selectedObject.position.z + 0.01);
    } else {
